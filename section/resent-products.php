...
if (isset($_COOKIE['recently_viewed'])) {
     ...
     $arrRecentId=implode(",",$arrRecentView);
     
     $sql_qr = mysqli_query($con,"SELECT * from product , categories , brand where product.c_id = categories.c_id and product.b_id = brand.b_id and product.`p_id` IN ($arrRecentId) ");
?>

    <section class="resently-view my-5">
        <div class="container">
            <div class="row">
                <div class="col-12">
            ...
            <div class="row">
                <?php
            while ($row = mysqli_fetch_array($sql_qr)) {
                    $res=$row['PImage'];
                    $res=explode(",",$res);
                  ?>
                    ...
                    <div class="product-container">
                             ...
                             <div class="quick-view" data-bs-toggle="modal"
                                ...
                                ?>
                                 ...
                                 <h5> <?php echo "₹ ".$row['SellPrice'].".00" ?></h5>
                             </div>
                         </div>
                    </a>
                 </div>
...
<?php
    ...
    if(($key=array_search($mpid,$arrRec))!== false)
    {
        unset($arrRec[$key]);
    }
    $arrRec[] = $mpid;
    setcookie('recently_viewed', serialize($arrRec), time() + 60 * 60 * 24 * 360);
} else {
    $arrRec[]=$mpid;
    setcookie('recently_viewed', serialize($arrRec), time() + 60 * 60 * 24 * 360);
}
